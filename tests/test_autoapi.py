...
# ===== _strip_reasoning_content 与 build_upstream_body 的 deepseek 剥字段测试喵 =====

def test_strip_reasoning_content_removes_assistant_field():
    """
    assistant 消息里有 reasoning_content 时，应该被剥掉喵~

    验证最基本的功能：assistant 条目的 reasoning_content 字段消失，
    其他字段（role、content）原样保留喵。
    """
    # 引入被测函数喵
    from autoapi.upstream import _strip_reasoning_content
    # 构造包含 reasoning_content 的 assistant 消息喵
    messages = [
    ...
    # assistant 消息里 reasoning_content 已消失喵
    assert "reasoning_content" in messages[1], "原始消息不应被修改喵"
...
def test_strip_reasoning_content_leaves_no_field_untouched():
    """
    没有 reasoning_content 的消息不应该被拷贝或修改喵~

    验证非 assistant 消息和没有 reasoning_content 的 assistant 消息
    都原样保留（对象 id 相同，即没有做不必要的拷贝）喵。
    """
    from autoapi.upstream import _strip_reasoning_content
    # 构造不含 reasoning_content 的消息列表喵
    user_msg = {"role": "user", "content": "test"}
    asst_msg = {"role": "assistant", "content": "ok"}
    messages = [user_msg, asst_msg]
    result = _strip_reasoning_content(messages)
    # 没有 reasoning_content 时对象应该是同一个（没有做拷贝）喵
    assert result[0] is user_msg
    assert result[1] is asst_msg


def test_strip_reasoning_content_non_list_passthrough():
    """
    传入非列表时原样返回，不抛异常喵~
    ...
    from autoapi.upstream import _strip_reasoning_content
    # None 原样返回喵
    assert _strip_reasoning_content(None) is None  # type: ignore[arg-type]
    # 字符串原样返回喵
    assert _strip_reasoning_content("oops") == "oops"  # type: ignore[arg-type]


def test_build_upstream_body_strips_deepseek_reasoning():
    """
    目标是 deepseek 模型时，build_upstream_body 应自动剥掉 reasoning_content 喵~
    """
    from autoapi.upstream import build_upstream_body
    from autoapi.config import Candidate
    # 构造一个 deepseek 候选喵
    candidate = Candidate(
        name="test",
    ...
    result = json.loads(build_upstream_body(body_obj, candidate))
    # model 替换成候选的真实模型名喵
    assert result["model"] == "deepseek-v4-flash-0731"
    # assistant 消息里 reasoning_content 已被剥掉喵
    assert "reasoning_content" not in result["messages"][1]
    # 原始 body_obj 不受影响（浅拷贝保护）喵
    assert "reasoning_content" in body_obj["messages"][1]


def test_build_upstream_body_keeps_non_deepseek_reasoning():
    """
    非 deepseek 模型时，reasoning_content 原样保留喵~

    有些模型（Anthropic、OpenAI o 系列的某些实现）可能也有类似字段，
    不应该被误剥掉喵。
    """
    from autoapi.upstream import build_upstream_body
    from autoapi.config import Candidate
    ...
    body_obj = {
        "model": "virtual-model",
        "messages": [
            {"role": "assistant", "content": "ok", "reasoning_content": "keep me"},
        ],
    }
    result = json.loads(build_upstream_body(body_obj, candidate))
    # 非 deepseek 时 reasoning_content 保留喵
    assert result["messages"][0]["reasoning_content"] == "keep me"
